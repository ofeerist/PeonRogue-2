...
float3 dummyNormal = 1;
half4 dummyInstanceColor = 0;
bool clipped = false;
bendGrass (positionWS, input.positionOS, dummyNormal, input.color, dummyInstanceColor, clipped);
if (clipped) {
    output.positionCS = input.positionOS.xxxx / (1-clipped);
    return output;
}
...
#include "Includes/ATG Instanced Indirect Inputs.hlsl"
            ...
            //  Here we should not tweak the normal but take it from the mesh as is.
            //  Otherwise SSAO goes crazy.
                // #if defined(_NORMAL)
                //     input.normalOS = terrainNormal;
                // #else
                //     input.normalOS = half3(0,1,0);
            ...
            //  Bend
                half4 dummyInstanceColor = 0;
                bool clipped = false;
                bendGrass (vertexInput.positionWS, input.positionOS, input.normalOS, input.color, dummyInstanceColor, clipped);            
                if (clipped) {
                    output.positionCS = input.positionOS.xxxx / (1-clipped);
                    return output;
                }
            ...
            //  We have to recalculate ClipPos!
                output.positionCS = TransformWorldToHClip(vertexInput.positionWS);
            //  DepthNormal need normal as well
                output.normalWS = NormalizeNormalPerVertex(normalInput.normalWS);
                return output;
            }
